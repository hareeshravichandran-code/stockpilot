...
router.get('/sync/pull', authMiddleware, async (req, res) => {
  try {
    ...
    const [txnRes, catRes, budgetRes, rulesRes] = await Promise.all([
    ...
    // sms_rules table may not exist yet — ignore error gracefully
    const rulesData = rulesRes.error ? [] : (rulesRes.data || []);

    // Map column names to camelCase for Android DTO
    ...
    res.json({
      ...
      sms_rules:    mappedRules,
      total_count:  (txnRes.data?.length || 0) + (catRes.data?.length || 0) +
                    (budgetRes.data?.length || 0) + mappedRules.length,
